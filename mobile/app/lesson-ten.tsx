...
import { LessonTenPage } from '@/components/lessons/LessonTenPage';
...
function idempotencyKey(blockId: string) {
  return `lesson-ten-${blockId}-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 8)}`;
}

export default function LessonTenScreen() {
  ...
  const lesson = useMemo(() => {
    ...
    return lessons.find((item) => item.declaredNumber === 10)
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setError(err instanceof ApiError ? err.message : '10-darsni yuklab bo‘lmadi');
    } finally {
      setIsLoading(false);
    }
  }, [sourceKey]);
  ...
  const nextLesson = useMemo(() => {
    ...
    return courseData.lessons
      .filter((item) => (item.declaredNumber ?? Number.MAX_SAFE_INTEGER) > 10)
  ...
  const completeLesson = async () => {
    ...
    try {
      ...
      setError(err instanceof ApiError ? err.message : '10-dars natijasini saqlab bo‘lmadi');
    } finally {
      setIsSaving(false);
    }
  };

  if (isLoading && !details.length) return <Screen><LoadingState text="10-dars yuklanmoqda…" /></Screen>;
  if (error && !details.length) return <Screen><ErrorState message={error} onRetry={() => void load()} /></Screen>;
  if (!lesson) return <Screen><ErrorState message="10-dars materiali topilmadi." /></Screen>;
