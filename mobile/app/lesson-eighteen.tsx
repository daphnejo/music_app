...
import { LessonEighteenPage } from '@/components/lessons/LessonEighteenPage';
...
function idempotencyKey(blockId: string) {
  return `lesson-eighteen-${blockId}-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 8)}`;
}

export default function LessonEighteenScreen() {
  ...
  const lesson = useMemo(() => {
    ...
    return lessons.find((item) => item.declaredNumber === 18)
      ?? lessons.find((item) => item.blocks.some((block) => block.id === blockId));
  }, [blockId, courseData]);

  const sourceBlockIds = useMemo(() => {
    if (lesson?.blocks.length) return lesson.blocks.slice(0, 4).map((block) => block.id);
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setError(err instanceof ApiError ? err.message : '18-darsni yuklab bo‘lmadi');
    } finally {
      setIsLoading(false);
    }
  }, [sourceKey]);
  ...
  const audios = useMemo(
    () => details[0]?.assets.filter((asset) => asset.kind === 'audio').slice(0, 7) ?? [],
    [details],
  );
  ...
  const nextLesson = useMemo(() => {
    ...
    return courseData.lessons
      .filter((item) => (item.declaredNumber ?? Number.MAX_SAFE_INTEGER) > 18)
  ...
  const completeLesson = async () => {
    ...
    try {
      ...
      setError(err instanceof ApiError ? err.message : '18-dars natijasini saqlab bo‘lmadi');
    } finally {
      setIsSaving(false);
    }
  };

  if (isLoading && !details.length) return <Screen><LoadingState text="18-dars yuklanmoqda…" /></Screen>;
  if (error && !details.length) return <Screen><ErrorState message={error} onRetry={() => void load()} /></Screen>;
  if (!lesson) return <Screen><ErrorState message="18-dars materiali topilmadi." /></Screen>;
