...
import { LessonElevenPage } from '@/components/lessons/LessonElevenPage';
...
function idempotencyKey(blockId: string) {
  return `lesson-eleven-${blockId}-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 8)}`;
}

export default function LessonElevenScreen() {
  ...
  const lesson = useMemo(() => {
    ...
    return lessons.find((item) => item.declaredNumber === 11)
      ?? lessons.find((item) => item.blocks.some((block) => block.id === blockId));
  }, [blockId, courseData]);

  const sourceBlockIds = useMemo(() => {
    if (lesson?.blocks.length) return lesson.blocks.slice(0, 3).map((block) => block.id);
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setError(err instanceof ApiError ? err.message : '11-darsni yuklab bo‘lmadi');
    } finally {
      setIsLoading(false);
    }
  }, [sourceKey]);
  ...
  const audios = useMemo(
    () => details.flatMap((detail) => detail.assets.filter((asset) => asset.kind === 'audio')),
    [details],
  );
  ...
  const nextLesson = useMemo(() => {
    ...
    return courseData.lessons
      .filter((item) => (item.declaredNumber ?? Number.MAX_SAFE_INTEGER) > 11)
  ...
  const completeLesson = async () => {
    ...
    try {
      ...
      setError(err instanceof ApiError ? err.message : '11-dars natijasini saqlab bo‘lmadi');
    } finally {
      setIsSaving(false);
    }
  };

  if (isLoading && !details.length) return <Screen><LoadingState text="11-dars yuklanmoqda…" /></Screen>;
  if (error && !details.length) return <Screen><ErrorState message={error} onRetry={() => void load()} /></Screen>;
  if (!lesson) return <Screen><ErrorState message="11-dars materiali topilmadi." /></Screen>;
