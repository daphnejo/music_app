...
import { LessonSevenPage } from '@/components/lessons/LessonSevenPage';
...
function idempotencyKey(blockId: string) {
  return `lesson-seven-${blockId}-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 8)}`;
}

export default function LessonSevenScreen() {
  ...
  const lesson = useMemo(() => {
    ...
    return lessons.find((item) => item.declaredNumber === 7)
      ?? lessons.find((item) => item.blocks.some((block) => block.id === blockId));
  }, [blockId, courseData]);

  const sourceBlockIds = useMemo(() => {
    if (lesson?.blocks.length) return lesson.blocks.slice(0, 6).map((block) => block.id);
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setError(err instanceof ApiError ? err.message : '7-darsni yuklab bo‘lmadi');
    } finally {
      setIsLoading(false);
    }
  }, [sourceKey]);
  ...
  const octaveAudios = useMemo(
    () => details[0]?.assets.filter((asset) => asset.kind === 'audio') ?? [],
    [details],
  );

  const singingAudios = useMemo(
    () => details.slice(1, 4).flatMap((detail) => detail.assets.filter((asset) => asset.kind === 'audio')),
    [details],
  );

  const videos = useMemo(
    () => details.flatMap((detail) => detail.assets.filter((asset) => asset.kind === 'video')),
    [details],
  );
  ...
  const nextLesson = useMemo(() => {
    ...
    return courseData.lessons
      .filter((item) => (item.declaredNumber ?? Number.MAX_SAFE_INTEGER) > 7)
  ...
  const completeLesson = async () => {
    ...
    try {
      ...
      setError(err instanceof ApiError ? err.message : '7-dars natijasini saqlab bo‘lmadi');
    } finally {
      setIsSaving(false);
    }
  };

  if (isLoading && !details.length) return <Screen><LoadingState text="7-dars yuklanmoqda…" /></Screen>;
  if (error && !details.length) return <Screen><ErrorState message={error} onRetry={() => void load()} /></Screen>;
  if (!lesson) return <Screen><ErrorState message="7-dars materiali topilmadi." /></Screen>;

  return (
    <Screen>
      <LessonSevenPage
        octaveAudios={octaveAudios}
        singingAudios={singingAudios}
        videos={videos}
        completed={completed}
        saving={isSaving}
        onBack={() => router.back()}
