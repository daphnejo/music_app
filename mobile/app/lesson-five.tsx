...
function idempotencyKey(blockId: string) {
  return `lesson-five-${blockId}-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 8)}`;
}

export default function LessonFiveScreen() {
  ...
  const lesson = useMemo(() => {
    ...
    return lessons.find((item) => item.declaredNumber === 5)
  ...
  const nextLesson = useMemo(() => {
    ...
    return courseData.lessons
      .filter((item) => (item.declaredNumber ?? Number.MAX_SAFE_INTEGER) > 5)
  ...
  const completeLesson = async () => {
    ...
    try {
      ...
      setError(err instanceof ApiError ? err.message : '5-dars natijasini saqlab bo‘lmadi');
    } finally {
      setIsSaving(false);
    }
  };
