    ...
    <Screen>
      ...
      <Pressable onPress={() => router.push('/piano')} style={styles.pianoCard}>
        <View style={styles.pianoTop}>
          <View style={[styles.pianoIcon, { backgroundColor: colors.primary }]}>
      ...
      <View style={styles.list}>
        {exercises.map(({ block, lesson }) => {
          ...
          return (
            ...
            >
              <View style={[styles.icon, { backgroundColor: colors.primarySoft }]}><Ionicons name={meta.icon} size={25} color={colors.primary} /></View>
              <View style={styles.body}>
                <Text style={[styles.kind, { color: colors.primary }]}>{meta.label}</Text>
              ...
              <View style={[styles.state, { backgroundColor: done ? colors.success : colors.primarySoft }]}>
                <Ionicons name={done ? 'checkmark' : 'chevron-forward'} size={17} color={done ? '#fff' : colors.primary} />
              </View>
            </Pressable>
          );
        })}
...
const styles = StyleSheet.create({
  ...
  pianoIcon: { width: 46, height: 46, borderRadius: 15, alignItems: 'center', justifyContent: 'center' },
  pianoBody: { flex: 1 },
  ...
  body: { flex: 1, gap: 3 },
  kind: { fontSize: 10, fontWeight: '900', letterSpacing: .4 },
  title: { fontSize: 15, lineHeight: 20, fontWeight: '800' },
  subtitle: { fontSize: 11, lineHeight: 16 },
  state: { width: 32, height: 32, borderRadius: 11, alignItems: 'center', justifyContent: 'center' },
});
