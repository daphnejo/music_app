  ...
  return (
    <Screen>
      ...
      <View style={styles.card}>
        <Field label="Joriy parol" value={currentPassword} onChangeText={setCurrentPassword} />
        <Field label="Yangi parol" value={newPassword} onChangeText={setNewPassword} hint="Kamida 8 belgi" />
        <Field label="Yangi parolni takrorlang" value={confirmPassword} onChangeText={setConfirmPassword} />
...
function Field({ label, value, onChangeText, hint }: { label: string; value: string; onChangeText: (value: string) => void; hint?: string }) {
  return <View style={styles.field}><Text style={styles.label}>{label}</Text><TextInput value={value} onChangeText={onChangeText} secureTextEntry autoCapitalize="none" autoCorrect={false} style={styles.input} /><Text style={styles.hint}>{hint ?? ' '}</Text></View>;
}
...
const styles = StyleSheet.create({
  ...
  input: { height: 52, borderRadius: 16, borderWidth: 1, borderColor: colors.border, backgroundColor: '#FAFAFE', paddingHorizontal: 14, color: colors.text, fontSize: 15 },
