...
import { LessonTwelvePage } from '@/components/lessons/LessonTwelvePage';
...
function idempotencyKey(blockId: string) {
  return `lesson-twelve-${blockId}-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 8)}`;
}

export default function LessonTwelveScreen() {
  ...
  const lesson = useMemo(() => {
    ...
    return lessons.find((item) => item.declaredNumber === 12)
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setError(err instanceof ApiError ? err.message : '12-darsni yuklab bo‘lmadi');
    } finally {
      setIsLoading(false);
    }
  }, [sourceKey]);
  ...
  const videos = useMemo(() => details.flatMap((detail) => detail.assets.filter((asset) => asset.kind === 'video')), [details]);
  const audios = useMemo(() => details.flatMap((detail) => detail.assets.filter((asset) => asset.kind === 'audio')), [details]);
  ...
  const nextLesson = useMemo(() => {
    ...
    return courseData.lessons
      .filter((item) => (item.declaredNumber ?? Number.MAX_SAFE_INTEGER) > 12)
  ...
  const completeLesson = async () => {
    ...
    try {
      ...
      setError(err instanceof ApiError ? err.message : '12-dars natijasini saqlab bo‘lmadi');
    } finally {
      setIsSaving(false);
    }
  };

  if (isLoading && !details.length) return <Screen><LoadingState text="12-dars yuklanmoqda…" /></Screen>;
  if (error && !details.length) return <Screen><ErrorState message={error} onRetry={() => void load()} /></Screen>;
  if (!lesson) return <Screen><ErrorState message="12-dars materiali topilmadi." /></Screen>;
