...
const REWARD_STEP = 5;
const TOTAL_STEPS = 6;
const TRACE_TARGET = 58;

const TRACE_GUIDE: Point[] = [
  { x: 0.58, y: 0.06 }, { x: 0.66, y: 0.13 }, { x: 0.67, y: 0.21 }, { x: 0.61, y: 0.29 },
  { x: 0.49, y: 0.35 }, { x: 0.37, y: 0.39 }, { x: 0.29, y: 0.47 }, { x: 0.29, y: 0.56 },
  { x: 0.36, y: 0.64 }, { x: 0.48, y: 0.69 }, { x: 0.60, y: 0.68 }, { x: 0.69, y: 0.61 },
  { x: 0.71, y: 0.53 }, { x: 0.65, y: 0.47 }, { x: 0.55, y: 0.45 }, { x: 0.46, y: 0.49 },
  { x: 0.43, y: 0.57 }, { x: 0.47, y: 0.64 }, { x: 0.56, y: 0.69 }, { x: 0.59, y: 0.78 },
  { x: 0.58, y: 0.87 }, { x: 0.53, y: 0.94 }, { x: 0.45, y: 0.95 }, { x: 0.40, y: 0.91 },
];
...
export function LessonFivePageV2({
  images,
  completed,
  saving,
  onBack,
  ...
  const guidePixels = useMemo(
    () => TRACE_GUIDE.map((point) => ({ x: point.x * canvasSize.width, y: point.y * canvasSize.height })),
  ...
  const tracePoint = useCallback((x: number, y: number) => {
    ...
    if (last && Math.hypot(last.x - x, last.y - y) < 5) return;

    const next = [...drawnRef.current, { x, y }].slice(-260);
    drawnRef.current = next;
    setDrawnPoints(next);

    guidePixels.forEach((point, index) => {
      if (Math.hypot(point.x - x, point.y - y) <= 42) visitedRef.current.add(index);
    });
  ...
  useEffect(() => {
    ...
    const timer = setInterval(() => setHintIndex((value) => (value + 1) % TRACE_GUIDE.length), 210);
    const animation = Animated.loop(Animated.sequence([
    ...
    <View style={styles.page}>
      ...
      {step === 1 ? (
        <View style={[styles.card, { backgroundColor: colors.surface, borderColor: colors.border }]}> 
          ...
          <View style={styles.tipBox}><Text style={styles.tipEmoji}>💡</Text><Text style={styles.tipText}>Mana shu asl namunani eslab qol: kalit doim nota yo‘lining boshida turadi.</Text></View>
        </View>
      ) : null}
      ...
      {step === 3 ? (
        <View style={[styles.traceCard, { backgroundColor: colors.surface, borderColor: colors.border }]}> 
          ...
          <Text style={[styles.sectionText, { color: colors.muted }]}>Xira namunaga qarab pushti nuqtalar ustidan barmog‘ingni yurgiz. ☝️ yo‘lni ko‘rsatadi.</Text>
          ...
          <GestureDetector gesture={drawingGesture}>
            <View style={styles.traceStage} onLayout={(event) => setCanvasSize({ width: event.nativeEvent.layout.width, height: event.nativeEvent.layout.height })}>
              {firstImage ? <Image pointerEvents="none" source={{ uri: firstImage }} resizeMode="contain" style={styles.traceReferenceImage} /> : null}
              <View pointerEvents="none" style={styles.traceGlow} />
              {guidePixels.map((point, index) => (
                <View key={index} pointerEvents="none" style={[styles.guideDot, { left: point.x - 3.5, top: point.y - 3.5, opacity: visitedRef.current.has(index) ? 0.18 : 0.78 }]} />
              ))}
              ...
              {!traceComplete && canvasSize.width > 1 ? (
                <Animated.View pointerEvents="none" style={[styles.hintFinger, { left: hintPoint.x - 16, top: hintPoint.y - 32, transform: [{ scale: hintPulse }] }]}><Text style={styles.hintFingerText}>☝️</Text></Animated.View>
              ) : null}
              <View pointerEvents="none" style={styles.startBadge}><Text style={styles.startBadgeText}>Shu yerdan boshla</Text></View>
            </View>
          </GestureDetector>
          ...
          <View style={styles.traceActions}>
            <View style={[styles.traceProgress, traceComplete && styles.traceProgressDone]}>
              ...
              <Text style={[styles.traceProgressText, traceComplete && styles.traceProgressTextDone]}>{traceComplete ? 'Ajoyib! Kalitni chizding.' : `Yo‘lning ${traceCoverage}% qismini chizding`}</Text>
            </View>
            <Pressable onPress={resetTrace} style={styles.resetButton}><Ionicons name="refresh" size={19} color="#C14E70" /></Pressable>
          </View>
          {practiceImage ? <View style={styles.practiceReference}><Text style={styles.practiceLabel}>Taqdimotdagi mashq namunasi</Text><Image source={{ uri: practiceImage }} resizeMode="contain" style={styles.practiceImage} /></View> : null}
        </View>
      ) : null}
        ...
        <View style={[styles.card, { backgroundColor: colors.surface, borderColor: colors.border }]}> 
          ...
          {quizChecked ? (
            <View style={[styles.feedback, { backgroundColor: answerCorrect ? colors.successSurface : '#FFF3D5' }]}>
              ...
              <View style={{ flex: 1 }}><Text style={[styles.feedbackTitle, { color: colors.text }]}>{answerCorrect ? 'To‘g‘ri!' : 'Yaxshi urinish!'}</Text><Text style={[styles.feedbackText, { color: colors.muted }]}>{answerCorrect ? 'Ha! Avval musiqa kaliti qo‘yiladi.' : 'To‘g‘ri javob — musiqa kaliti.'}</Text></View>
            </View>
          ) : null}
        </View>
      ) : null}
      ...
      {step === REWARD_STEP ? (
        <View style={styles.rewardCard}>
          <Animated.View style={{ alignItems: 'center', transform: [{ scale: rewardScale }] }}>
            ...
            <Text style={[styles.rewardText, { color: colors.muted }]}>Skripka kalitini taniy olasan va uni barmog‘ing bilan izlab chizding!</Text>
      ...
      <Pressable disabled={buttonDisabled} onPress={goForward} style={[styles.completeButton, { backgroundColor: isFinalStep ? colors.success : '#C14E70' }, buttonDisabled && styles.disabled]}>
...
const styles = StyleSheet.create({
  ...
  traceCard: { minHeight: 500, borderRadius: 32, padding: 20, borderWidth: 1, justifyContent: 'center' },
  ...
  traceStage: { height: 320, borderRadius: 26, marginTop: 18, backgroundColor: '#FFF8FA', borderWidth: 1.5, borderColor: '#F5C7D4', overflow: 'hidden', position: 'relative' },
  traceReferenceImage: { position: 'absolute', left: '13%', top: '5%', width: '74%', height: '90%', opacity: 0.13 },
  traceGlow: { position: 'absolute', left: '24%', top: '9%', width: '52%', height: '82%', borderRadius: 100, backgroundColor: '#FFE7EE', opacity: 0.32 },
  guideDot: { position: 'absolute', width: 7, height: 7, borderRadius: 4, backgroundColor: '#C14E70' },
  drawDot: { position: 'absolute', width: 11, height: 11, borderRadius: 6, backgroundColor: '#C14E70', shadowColor: '#C14E70', shadowOpacity: 0.34, shadowRadius: 4, elevation: 2 },
